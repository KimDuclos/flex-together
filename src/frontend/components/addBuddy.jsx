...
class AddBuddy extends React.Component {
	...
	handleNext = e => {
		...
		axios
			.post('https://flextogether.herokuapp.com/api/invite', request)
			.then(result => {
	...
	render() {
		return (
			<div className="addBuddyText">
				...
				<p>
					Invite a friend of a loved one to complete the program with you. We'll show them your available
					times and find a time that works for both of you.
				</p>
				...
				<button
					...
					value="Medium"
					onClick={this.handleMobility}
					style={{ backgroundColor: this.state.bgColor2, color: this.state.textColor2 }}
				>
					Medium
				</button>
				<button
					...
					value="High"
					onClick={this.handleMobility}
					style={{ backgroundColor: this.state.bgColor3, color: this.state.textColor3 }}
				>
					High
				</button>
				<br />
