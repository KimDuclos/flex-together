	...
	render() {
		...
		if (selTimes === null) {
			...
			availabilityDisplay = selectedTimes.map(selected => {
				const timeBlocks = selected.times.map(
					time => <TimeButtons time={time} day={selected.day} clickHandler={this.handleClick} />
					// <button
					//     className='timeBlock'
				...
				return (
					<div className="buddyTimeChoices" key={selected.day}>
						<label>
							{selected.day}
						</label>
						<br />
		...
		return (
			<div className="bigText">
				<h3>
					Here are some good times for {userName}, do any of these work for you?
				</h3>
				<p>
					To complete the beta, we are asking that people commit to one 30-minute time block once per week.
					Please choose a time that works well for you.
				</p>
