...
router.post('/register', (req, res) => {
	...
	if (
		!req.body.myInfo.userType ||
		!req.body.myInfo.name ||
		!req.body.myInfo.email ||
		!req.body.myInfo.phone ||
		req.body.myInfo.notifyEmail === null ||
		!req.body.myInfo.notifyPhone === null ||
		!req.body.myInfo.mobility ||
		!req.body.myInfo.availabilityTimes
	) {
		res.status(400).json({
			error:
				'Must include userType, name, email, phone, notifyEmail, notifyPhone, mobility, and availabilityTimes in body.myInfo'
		});
		return;
	}

	if (
		!req.body.inviteInfo.name ||
		!req.body.inviteInfo.email ||
		!req.body.inviteInfo.phone ||
		!req.body.inviteInfo.mobility
	) {
		res.status(400).json({
	...
	const emailPromise = verifyToken =>
		emailer.sendMessage(user_a.email, 'Your FlexTogether Verification Token', verifyToken);

	userAPromise()
		.then(([id]) => {
			user_ids.push(id);
			times = times.map(time => {
				return { user_id: id, ...time };
			});
			return userBPromise();
		})
		.then(([id]) => {
		...
		.then(invitees => {
			return Promise.all(
				invitees.map(({ invitee_id }) => {
					...
					const sendEmailPromise = invitee_email =>
						emailer.sendMessage(invitee_email, 'Your FlexTogether Invite Token', token);
					const getPhonePromise = () => db.getUserPhone(invitee_id);
					const sendPhonePromise = invitee_phone =>
						texter.sendMessage(invitee_phone, `Your FlexTogether Invite Token: ${token}`);
...
router.get('/confirmInvite/:token', (req, res) => {
	...
	Promise.all([
		db.getUserName(data.inviter_id),
		db.getUserTimes(data.inviter_id),
		db.verifyUserEmail(data.invitee_id)
	])
		.then(([name, times]) => {
